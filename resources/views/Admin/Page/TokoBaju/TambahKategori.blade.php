...
<div class="card mb-3">
    <div class="card-body">
        ...
        <form action="{{ route('storeKategori') }}" method="POST">
            @csrf
            <div class="mb-3">
                <label for="kategoriTokobaju" class="form-label">Tambah Kategori:</label>
                <input type="text" class="form-control" id="kategoriTokobaju" name="kategoriTokobaju" placeholder="Contoh: Pakaian Pria">
            </div>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                @foreach($kategoriTokobaju as $index => $kategori)
                    <tr>
                        <th scope="row">{{ $index + 1 }}</th>
                        <td>{{ $kategori->name }}</td>
                        <td>
                            <div class="d-flex justify-content-center align-items-center">
                                <a href="{{ route('editKategori', $kategori->id) }}" class="text-warning me-2">
                                    <i class="bi bi-pencil-square" style="font-size: 20px; cursor: pointer;"></i>
                                </a>
                                <form action="{{ route('deleteKategori', $kategori->id) }}" method="POST" id="deleteForm{{ $kategori->id }}">
                                    @csrf
                                    @method('DELETE')
...
<script src="{{ asset('js/kategoriTokobaju.js') }}"></script>
@endsection
